...
from pong_ai.es import QNetwork
...
def train(
    ...
    # Oponente: rede congelada compartilhada pelos envs; recarregada do HOF
    opponent = QNetwork()
    opponent.load_state_dict(agent.policy.state_dict())
    opponent.eval()
    opponent.requires_grad_(False)

    hof: list[dict] = [copy.deepcopy(agent.policy.state_dict())]
    ...
    envs = SyncVectorEnv([lambda: PongEnv(opponent=opponent) for _ in range(NUM_ENVS)])
    ...
    while agent.global_step < target and not (stop_event is not None and stop_event.is_set()):
        ...
        for t in range(ROLLOUT_STEPS):
            ...
            for k in np.flatnonzero(dones):
                completed.append(float(episode_rewards[k]))
        ...
        if agent.global_step >= next_snapshot:
            ...
            if len(hof) > HOF_MAX_SIZE:
                hof.pop(1)  # descarta o mais antigo, preserva a âncora inicial
            opponent.load_state_dict(random.choice(hof))
            next_snapshot += HOF_SNAPSHOT_EVERY
            logger.info(
                "HOF snapshot at step %d (archive %d) — opponent swapped",
                agent.global_step, len(hof),
            )

        agent.save(str(MODEL_PATH))

        if on_rollout is not None:
            on_rollout(agent, target, completed)

        rollout_count += 1
        if rollout_count % LOG_EVERY == 0:
            recent = completed[-50:] if completed else [0.0]
            logger.info(
                "step %d/%d | avg_return=%.2f | episodes=%d | "
                "policy_loss=%.4f | value_loss=%.4f | entropy=%.3f",
                agent.global_step, target,
                sum(recent) / len(recent), len(completed),
